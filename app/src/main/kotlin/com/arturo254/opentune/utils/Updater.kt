...
object Updater {
    ...
    private suspend fun fetchReleasesNetwork(
        ...
        val response: HttpResponse =
            client.get("https://api.github.com/repos/Erorr40/OpenTune/releases?per_page=$perPage") {
                headers {
    ...
    private suspend fun resolveApkDownloadUrl(tagName: String): String {
        val fallback =
            "https://github.com/Erorr40/OpenTune/releases/download/$tagName/$APK_ASSET_NAME"

        return runCatching {
            val response = client.get(
                "https://api.github.com/repos/Erorr40/OpenTune/releases/tags/$tagName"
            ) {
                headers {
                    append("Accept", "application/vnd.github+json")
                    append("User-Agent", "OpenTune")
    ...
    suspend fun getCommitHistory(count: Int = 20, branch: String = "master"): Result<List<GitCommit>> =
        runCatching {
            val response =
                client.get("https://api.github.com/repos/Erorr40/OpenTune/commits?sha=$branch&per_page=$count") {
                    headers {
                        append("Accept", "application/vnd.github+json")
                        append("User-Agent", "OpenTune")
                    }
    ...
    fun getLatestDownloadUrl(): String {
        ...
        return when (channel) {
            UpdateChannel.STABLE -> {
                "https://github.com/Erorr40/OpenTune/releases/latest/download/$APK_ASSET_NAME"
            }
            UpdateChannel.NIGHTLY -> {
